...
class MqttServiceClient implements MqttCallback {
	 ...
	 */
	MqttServiceClient(MqttService service, String serverURI, String clientId, MqttClientPersistence persistence,
			String clientHandle) {
	...
	public void connect(MqttConnectOptions options, String invocationContext, String activityToken) {
		...
		try {
			...
			if (persistence == null) {
				...
				if (myDir == null) {
					resultBundle
							.putString(MqttServiceConstants.CALLBACK_ERROR_MESSAGE, "No external storage available");
			...
			IMqttActionListener listener = new MqttServiceClientListener(resultBundle) {
	...
	// Iterator<StoredMessage> backlog =
	...
	// Log.e("MqttServiceClient deliverBacklog", msgArrived.getMessageId() + ":"
	// + new String(msgArrived.getMessage().getPayload()));
	// service.callbackToActivity(clientHandle, Status.OK, resultBundle);
	// }
	// }
	 ...
	 * @return the bundle
	 */
	private Bundle messageToBundle(String topic, MqttMessage message) {
	...
	public void deliveryComplete(IMqttDeliveryToken messageToken) {
		...
		if (message != null) { // If I don't know about the message, it's
			...
			if (message.getQos() != 0) {
				//// FIXME: 2016/4/15   to complete message send confirm
				Bundle resultBundle = messageToBundle(topic, message);
		...
		// previously sechudled ping anyw
	}
	...
	public void reInit(MqttService service, String serverURI, String clientId, MqttClientPersistence persistence,
			String clientHandle){
		this.serverURI = serverURI.toString();
		this.service = service;
		this.clientId = clientId;
