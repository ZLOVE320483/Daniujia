...
public class ListUtils {
    ...
    private static <V> boolean isEquals(V actual, V expected) {
        if (actual == null) {
            if (expected == null) {
                return true;
            }
     ...
     */
    public static <V> String join(List<V> list) {
     ...
     */
    public static <V> String join(List<V> list, char separator) {
     ...
     */
    public static <V> String join(List<V> list, String separator) {
        if (isEmpty(list)) {
            return "";
        }
        if (separator == null) {
        ...
        for (int i = 0; i < list.size(); i++) {
            joinStr.append(list.get(i).toString());
            if (i != list.size() - 1) {
    ...
    public static String[] separate(String listJoin) {
        return separate(listJoin, DEFAULT_JOIN_SEPARATOR);
    }

    public static String[] separate(String listJoin, String separator){
        if (TextUtils.isEmpty(listJoin)) {
            return null;
        }

        return listJoin.split(separator);
    }
     ...
     * @return
     */
    public static <V> List<String> getStringList(List<V> sourceList) {
        List<String> resuList = new ArrayList<String>();
        if (isEmpty(sourceList)) {
            return resuList;
        }
        for (V item : sourceList) {
            resuList.add(item.toString());
        }
        return resuList;
    }

    public static String sortJoin(List<String> sourceList) {
        List<Integer> content = new ArrayList<Integer>();
        if (isEmpty(sourceList)) {
            return "";
        }
        for (int i = 0; i < sourceList.size(); i++) {
            content.add(Integer.valueOf(sourceList.get(i)));
        }
