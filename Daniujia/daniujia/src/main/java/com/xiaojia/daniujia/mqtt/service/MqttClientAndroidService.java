...
public class MqttClientAndroidService extends BroadcastReceiver implements IMqttAsyncClient {
	...
	private static final String SERVICE_NAME = "com.xiaojia.daniujia.mqtt.service.MqttService";

	private static final int BIND_SERVICE_FLAG = 0;

	private boolean serviceBindSuccess = false;
	private boolean broadcastRegistSuccess = false;
	...
	private final class MyServiceConnection implements ServiceConnection {
		...
		public void onServiceDisconnected(ComponentName name) {
			// When the abnormal situation of automatic call
			mqttService = null;
	...
	private static MqttClientAndroidService instance;
	/**
	 * Constructor
	 * 
	 * @param context
	...
	public IMqttToken connect(MqttConnectOptions options, Object userContext, IMqttActionListener callback)
		...
		if (mqttService == null) { // First time - must bind to the service
			...
			// We bind with BIND_SERVICE_FLAG (0), leaving us the manage the
			// lifecycle
			// until the last time it is stopped by a call to stopService()
			serviceBindSuccess = myContext.bindService(serviceStartIntent, serviceConnection, BIND_SERVICE_FLAG);
			if (serviceBindSuccess && !broadcastRegistSuccess) {
				IntentFilter filter = new IntentFilter();
				filter.addAction(MqttServiceConstants.CALLBACK_TO_ACTIVITY);
				myContext.registerReceiver(this, filter);
				broadcastRegistSuccess = true;
			}
		} else {
	...
	public IMqttToken disconnect() throws MqttException {
		IMqttToken token = new MqttTokenAndroidService(this, null, (IMqttActionListener) null);
	...
	public IMqttDeliveryToken publish(String topic, byte[] payload, int qos, boolean retained, Object userContext,
		...
		MqttDeliveryTokenAndroidService token = new MqttDeliveryTokenAndroidService(this, userContext, callback,
				message);
		String activityToken = storeToken(token);
		if (mqttService != null) {
	...
	private void conversationArrive(Bundle data) {
	...
	private void connectionLostAction(Bundle data) {
		if (callback != null) {
			...
			LogUtil.d("ZLOVE", "connectionLostAction---reason---" + reason);
	...
	private void simpleAction(IMqttToken token, Bundle data) {
		if (token != null) {
			...
			if (status == Status.OK) {
				...
				((MqttTokenAndroidService) token).notifyFailure(exceptionThrown);
			}
		}
	}
	...
	public void doHandleExit() {
		if (serviceBindSuccess && serviceConnection != null) {
			myContext.unbindService(serviceConnection);
			myContext.unregisterReceiver(this);
			serviceBindSuccess = false;
			broadcastRegistSuccess = false;
		}
	}
}
