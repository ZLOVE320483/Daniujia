...
public class DepositBankFragment extends BaseFragment implements View.OnClickListener {
	...
	protected int getInflateLayout() {
		return R.layout.frag_balance_to_bank_card;
	}
	...
	protected void setUpView(View view) {
		...
		etCardName = (EditText) view.findViewById(R.id.name);
		etCardNo = (EditText) view.findViewById(R.id.card_no);
		etBankName = (EditText) view.findViewById(R.id.bank);
		etDepositMoney = (EditText) view.findViewById(R.id.money);
		btnConfirm = (Button) view.findViewById(R.id.confirm);
		btnConfirm.setOnClickListener(this);

		etDepositMoney.setHint("本次最多可提现" + balance + "元");
	...
	public void onClick(View v) {
		if (v == btnConfirm) {
			accountNo = etCardNo.getText().toString().trim();
	...
	private void setAccountInfo(DepositAccountRespVo respVo) {
		...
		etCardName.setText(respVo.getName());
		etCardNo.setText(respVo.getAccountNo());
		etBankName.setText(respVo.getBankName());
	}

	private void depositRequest() {
		String url = Config.WEB_API_SERVER + "/user/deposit/do";
		FormEncodingBuilder builder = new FormEncodingBuilder();
		builder.add("aType", "bank");
	...
	private void requestBankAccountInfo() {
		String url = Config.WEB_API_SERVER + "/user/deposit/account/2";
