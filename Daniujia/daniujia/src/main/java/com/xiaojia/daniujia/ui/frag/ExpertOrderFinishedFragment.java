 ...
 */
public class ExpertOrderFinishedFragment extends BaseFragment implements AdapterView.OnItemClickListener, XListView.IXListViewListener {
    ...
    public void refreshData(MyAppointExpert result) {
        ...
        if (result == null) {
            ...
            tvNoDataTip.setText("您还没有已结束的服务订单");
            return;
        } else {
    ...
    public void requestData() {
        String url = Config.WEB_API_SERVER_V3 + "/user/order/need/serve/list/" + SysUtil.getIntPref(PrefKeyConst.PREF_USER_ID) + "/3/" + pageNum;
