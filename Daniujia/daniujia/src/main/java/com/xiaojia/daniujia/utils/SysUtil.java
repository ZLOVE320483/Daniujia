...
public class SysUtil {
	private static final String PREF_NAME = "dnj_pref";
	...
	public static String formatFloatNum(String num) {
	...
	public static String formatFloatNum(float price) {
		DecimalFormat decimalFormat = new DecimalFormat("0.00");//构造方法的字符格式这里如果小数不足2位,会以0补足.
		String p = decimalFormat.format(price);//format 返回的是字符串
		return p;
	}
	...
	public static void saveUserInfo(UserInfo userInfo) {
		savePref(PrefKeyConst.PREF_IS_LOGIN, true);
		savePref(PrefKeyConst.PREF_USER_ID, userInfo.getUserId());
		savePref(PrefKeyConst.PREF_APPLY_STEP, userInfo.getApplyStep());
		savePref(PrefKeyConst.PREF_USER_NAME, userInfo.getUsername());
		savePref(PrefKeyConst.PREF_NAME, userInfo.getName());
		savePref(PrefKeyConst.PREF_USER_AVATAR, userInfo.getHead());
		savePref(PrefKeyConst.PREF_USER_IDENTITY, userInfo.getIdentity());
		savePref(PrefKeyConst.PREF_PASSWORD, userInfo.getPassword());
		savePref(PrefKeyConst.PREF_HASH_ID, userInfo.getHashId());
	}

	public static void clearUserInfo() {
		JPushInterface.setAlias(ApplicationUtil.getApplicationContext(), "", null);
		savePref(PrefKeyConst.PREF_IS_LOGIN, false);
		savePref(PrefKeyConst.PREF_TOKEN, "");
		savePref(PrefKeyConst.PREF_APPLY_STEP, 0);
		savePref(PrefKeyConst.PREF_isMulityLogin, false);
		savePref(PrefKeyConst.PREF_USER_ID, 0);
		savePref(PrefKeyConst.PREF_USER_NAME, "");
		savePref(PrefKeyConst.PREF_NAME, "");
		savePref(PrefKeyConst.PREF_USER_AVATAR, "");
		savePref(PrefKeyConst.PREF_USER_IDENTITY, 0);
		savePref(PrefKeyConst.PREF_PASSWORD, "");
		savePref(PrefKeyConst.PREF_HASH_ID, 0);
	}

	public static UserInfo getUsrInfo() {
		UserInfo userInfo = new UserInfo();
		userInfo.setUserId(getIntPref(PrefKeyConst.PREF_USER_ID));
		userInfo.setUsername(getPref(PrefKeyConst.PREF_USER_NAME));
		userInfo.setName(getPref(PrefKeyConst.PREF_NAME));
		userInfo.setHead(getPref(PrefKeyConst.PREF_USER_AVATAR));
		userInfo.setIdentity(getIntPref(PrefKeyConst.PREF_USER_IDENTITY));
		userInfo.setPassword(getPref(PrefKeyConst.PREF_PASSWORD));
		return userInfo;
	}
